...
const handleTransaction = async (req: unknown): Promise<void> => {
  ...
  // Handle different transaction types (pacs.002 uses FIToFIPmtSts, pacs.008 uses FIToFICstmrCdtTrf)
  // Use type assertion to handle both pacs.002 and pacs.008 message types
  const transaction = request.transaction as any;
  const createdDateTime = transaction.FIToFIPmtSts?.GrpHdr?.CreDtTm ||
                          transaction.FIToFICstmrCdtTrf?.GrpHdr?.CreDtTm;

  if (!createdDateTime) {
    loggerService.warn('Could not find CreDtTm in transaction, using current time',
      `${configuration.RULE_NAME}@${configuration.RULE_VERSION}`,
      configuration.functionName);
  }

  const transactionDate = new Date(createdDateTime || new Date().toISOString());
